...
class CombinedVisualizer:
    def __init__(self, points_array, connections, video_path):
        ...
        self.n_frames = min(
            len(points_array), int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))
        )
    ...
    def setup_controls(self):
        ...
        self.frame_slider = Slider(
            slider_ax, "Frame", 0, self.n_frames - 1, valinit=0, valstep=1
        )
    ...
    def play_pause(self, event):
        if self.playing:
            ...
            self.ani = FuncAnimation(
                self.fig, self.animation_update, frames=frames, interval=50, repeat=True
            )
...
x = np.load(
    "/home/enkwi/Desktop/Sapiens/mvp/data_npy/keypoints_boris_3d.npy", allow_pickle=True
)
connections = [
    [15, 13],
    [13, 11],
