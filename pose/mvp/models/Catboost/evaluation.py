...
def summarize_predictions(results):
    ...
    new_d = {value: key for key, value in names.items()}
    for file_name, data in file_predictions.items():
        ...
        if file_name[:-4] == new_d[most_common_pred]:
            correct_predictions += 1

    return summaries, correct_predictions
...
def catboost_inference(best_weights, eval_dir, scaler):
    ...
    model.load_model(str(best_weights))
...
def catboost_main(
    ...
    results, true_labels, all_preds = catboost_inference(best_weights, eval_dir, scaler)
    summaries, correct_predictions = summarize_predictions(results)
    ...
    report_csv_path = WEIGHTS / "catboost_classification_report.csv"
    report_df.to_csv(report_csv_path, index=True)
    print(f"\nClassification report saved to {report_csv_path}")
    print(
        f"\nCorrectly classified files: {correct_predictions} out of {len(summaries)}"
    )
